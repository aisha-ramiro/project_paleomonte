// Supabase Edge Function: convite seguro de administradores.
  ...
  try {
    ...
    if (body.action === 'invite_admin') {
      ...
      const { error: grantError } = await adminClient
        .from('user_roles')
        .upsert({ user_id: invitedId, role: 'admin', granted_by: user.id }, { onConflict: 'user_id,role' });
      if (grantError) throw grantError;

      return json({ user: { id: invitedId, email: invited.user.email }, message: 'Convite de administrador enviado.' }, 201);
    }
