...
function Header() {
  ...
  return <header className="site-header"><div className="shell header-inner"><Brand/><button className="menu-toggle" onClick={() => setOpen(!open)} aria-label="Abrir menu">☰</button><nav className={open ? 'open' : ''}>{navItems.map(([name, href]) => <a key={href} href={href} onClick={() => setOpen(false)}>{name}</a>)}</nav></div></header>;
}

function Footer() { return <footer className="footer"><div className="shell footer-grid"><Brand inverse/><section><h4>Links úteis</h4><a href="#/catalogo">Catálogo</a><a href="#/sobre">Sobre o Museu</a><a href="#/acessibilidade">Acessibilidade</a></section><section><h4>Acessibilidade</h4><a href="#/acessibilidade">Aumentar fonte <b>A+</b></a><a href="#/acessibilidade">Diminuir fonte <b>A−</b></a><a href="#/acessibilidade">Alto contraste</a></section><section><h4>Siga-nos</h4><div className="socials"><span>f</span><span>◎</span></div></section><a className="footer-admin-link" href="#/admin" aria-label="Painel Administrativo" title="Painel Administrativo"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M16 3h-1.3a3 3 0 0 0-5.4 0H8a3 3 0 0 0-3 3v14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V6a3 3 0 0 0-3-3Zm-4 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2Zm4 15H8v-1.5h8V18Zm0-4H8v-1.5h8V14Zm0-4H8V8.5h8V10Z"/></svg></a></div><div className="mosaic"/></footer>; }
...
function App() { const route = useRoute(); const passwordSetup = window.location.pathname.replace(/\/+$/, '') === '/definir-senha'; usePublicAccessTracking(passwordSetup ? '/admin' : route); const { specimens, loading, error, reload } = usePublicCatalog(); let content; if (passwordSetup) content = <AdminPanel onCatalogChanged={reload} passwordSetup/>; else if (route.startsWith('/catalogo')) content = <Catalog specimens={specimens} loading={loading} error={error}/>; else if (route.startsWith('/fosseis/')) content = <SpecimenPage specimen={specimens.find(s => route.includes(s.slug))} loading={loading}/>; else if (route === '/sobre') content = <About/>; else if (route === '/acessibilidade') content = <Accessibility/>; else if (route === '/admin') content = <AdminPanel onCatalogChanged={reload}/>; else content = <Home specimens={specimens} loading={loading}/>; const isAdmin = route === '/admin' || passwordSetup; return <>{!isAdmin && <Header/>}{content}{!isAdmin && <Footer/>}</>; }
