...
export const copy = {
  pt: {
    ...
    footer: { links: 'Links úteis', accessibility: 'Acessibilidade', follow: 'Siga-nos', increase: 'Aumentar fonte', decrease: 'Diminuir fonte', contrast: 'Alto contraste' },
    home: {
    ...
    accessibility: { eyebrow: 'Para todas as pessoas', title: 'Acessibilidade', intro: 'O PaleoMonte foi projetado para oferecer uma navegação simples, legível e acolhedora durante a visita ao museu.', increase: 'Aumentar texto', normal: 'Tamanho padrão', contrast: 'Alto contraste', cards: [['Navegação por teclado', 'Todos os elementos interativos podem ser acessados pelo teclado.'], ['Conteúdo em áudio', 'As espécies contam com controles para ouvir suas descrições.'], ['Leitura clara', 'Tipografia legível, contraste adequado e estrutura semântica.']] },
  },
  en: {
    ...
    accessibility: { eyebrow: 'For everyone', title: 'Accessibility', intro: 'PaleoMonte was designed to provide simple, readable, welcoming navigation during a museum visit.', increase: 'Increase text', normal: 'Default text size', contrast: 'High contrast', cards: [['Keyboard navigation', 'Every interactive element can be accessed with a keyboard.'], ['Audio content', 'Species include controls to listen to their descriptions.'], ['Clear reading', 'Legible type, suitable contrast, and semantic structure.']] },
  },
};

export function localizeSpecimen(specimen, language) {
  if (language !== 'en') return specimen;
  const translation = specimen.translations?.en ?? {};
  ...
  return {
    ...
    length: specimen.lengthMeters ? `${specimen.lengthMeters} meters` : '',
  };
}
