...
function SpecimenForm({ specimen, roles, onSaved, onCancel }) {
  ...
  const submit = async (event) => {
    ...
    const payload = {
      ...
      slug: form.slug || slugify(form.scientific_name),
      summary: form.summary.trim() || null,
      description: form.description.trim() || null,
      geological_period: form.geological_period.trim() || null,
      geological_era: form.geological_era.trim() || null,
      discovery_location: form.discovery_location.trim() || null,
      specimen_type: form.specimen_type.trim() || null,
      diet: form.diet.trim() || null,
      length_meters: form.length_meters === '' ? null : Number(form.length_meters),
      status: canPublish ? form.status : 'draft',
    };
    const query = specimen
      ? supabase.from('specimens').update(payload).eq('id', specimen.id).select().single()
      : supabase.from('specimens').insert({ ...payload, created_by: (await supabase.auth.getUser()).data.user.id }).select().single();
...
function SpeciesManager({ roles }) {
  ...
  const load = async () => {
    setLoading(true);
    const { data, error: loadError } = await supabase.from('specimens').select('id, scientific_name, common_name, slug, status, updated_at, geological_period').order('updated_at', { ascending: false });
    setLoading(false);
...
function MediaManager() {
  ...
  useEffect(() => { supabase.from('specimens').select('id, scientific_name').order('scientific_name').then(({ data }) => setSpecimens(data ?? [])); }, []);
  const submit = async (event) => {
    event.preventDefault();
    if (!file || !selectedSpecimen) { setError('Escolha uma espécie e um arquivo.'); return; }
    setUploading(true); setError(''); setMessage('');
    const type = file.type.startsWith('image/') ? 'image' : file.type.startsWith('audio/') ? 'audio' : 'document';
    const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '-');
    const storagePath = `${type}/${crypto.randomUUID()}-${safeName}`;
    ...
    const { data: media, error: mediaError } = await supabase.from('media').insert({ storage_path: storagePath, type, mime_type: file.type, size_bytes: file.size, alt_text: altText || null, transcript: transcript || null, created_by: (await supabase.auth.getUser()).data.user.id }).select().single();
    if (mediaError) { setUploading(false); setError(mediaError.message); return; }
    const purpose = type === 'audio' ? 'audio_description' : type === 'image' ? 'gallery' : 'document';
    const { error: linkError } = await supabase.from('specimen_media').insert({ specimen_id: selectedSpecimen, media_id: media.id, purpose });
    setUploading(false);
    if (linkError) { setError(linkError.message); return; }
    setMessage('Arquivo enviado como pendente de aprovação.'); setFile(null); setAltText(''); setTranscript('');
  };
  return <section className="admin-section"><div className="section-toolbar"><div><p className="eyebrow">Acervo digital</p><h2>Mídias</h2></div></div><form className="admin-form compact-form media-form" onSubmit={submit}><p>Envie fotos, áudios ou documentos para uma espécie. Os arquivos entram como pendentes até a aprovação da curadoria.</p><label>Espécie *<select value={selectedSpecimen} onChange={(event) => setSelectedSpecimen(event.target.value)} required><option value="">Selecione</option>{specimens.map((specimen) => <option value={specimen.id} key={specimen.id}>{specimen.scientific_name}</option>)}</select></label><label>Arquivo *<input type="file" accept="image/jpeg,image/png,image/webp,audio/mpeg,audio/ogg,audio/wav,application/pdf" onChange={(event) => setFile(event.target.files?.[0] ?? null)} required/></label><label>Texto alternativo da imagem<textarea rows="3" value={altText} onChange={(event) => setAltText(event.target.value)} placeholder="Obrigatório antes da aprovação de imagens."/></label><label>Transcrição do áudio<textarea rows="4" value={transcript} onChange={(event) => setTranscript(event.target.value)} placeholder="Obrigatória antes da aprovação de áudios."/></label>{error && <p className="form-error" role="alert">{error}</p>}{message && <p className="form-success">{message}</p>}<button className="button green" disabled={uploading}>{uploading ? 'Enviando...' : 'Enviar mídia'}</button></form></section>;
}
...
function QrCodesManager() {
  ...
  const [selectedSpecimen, setSelectedSpecimen] = useState('');
  const [error, setError] = useState('');
  const load = async () => { const [{ data: available }, { data: codes, error: codesError }] = await Promise.all([supabase.from('specimens').select('id, scientific_name, slug').order('scientific_name'), supabase.from('qr_codes').select('id, specimen_id, public_path, status, version, specimens(scientific_name)').order('created_at', { ascending: false })]); setSpecimens(available ?? []); setQrCodes(codes ?? []); if (codesError) setError(codesError.message); };
  useEffect(() => { load(); }, []);
  const create = async (event) => { event.preventDefault(); const specimen = specimens.find((item) => item.id === selectedSpecimen); if (!specimen) return; setError(''); const { error: saveError } = await supabase.from('qr_codes').insert({ specimen_id: specimen.id, public_path: `/fosseis/${specimen.slug}`, generated_by: (await supabase.auth.getUser()).data.user.id }); if (saveError) { setError(saveError.message); return; } setSelectedSpecimen(''); load(); };
...
function AdminShell({ session, roles, onSignOut }) {
  ...
  const canManage = roles.some((role) => rolesThatCanManageContent.includes(role));
  const canPublish = roles.some((role) => rolesThatCanPublish.includes(role));
  const labels = ['Painel', 'Espécies', ...(canManage ? ['Categorias', 'Mídias', 'QR Codes'] : []), ...(roles.includes('admin') ? ['Usuários'] : []), 'Configurações'];
